...
SELECT email,
       left(password_hash, 4)  AS hash_prefix,   -- expect $2b$
       length(password_hash)    AS hash_length   -- ~60
FROM auth.users
WHERE email = 'your@email.com';

-- verify the password if match

SELECT auth.verify_password_bcrypt('PlainPasswordYouUsed', password_hash) AS matches
FROM auth.users
WHERE email = 'your@email.com';
-- or get the user id only if it matches:
SELECT id
FROM auth.users
WHERE email = 'your@email.com'
  AND auth.verify_password_bcrypt('PlainPasswordYouUsed', password_hash);

 --confirm login was created
